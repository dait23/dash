...
renderAction(){
  ...
  if(window.localStorage.getItem('urole') === 'Bde'){
   ...
   return(
    ...
    return(
      <div>
       <Link to={`/place/view/${this.props.place.id}`} className="badge badge-info">View</Link>
       <Link to={`/brand/create/${this.props.place.id}`} className="badge badge-success">Create Workflow</Link>
      </div>
    )
  }
}
